...
gain = 1.
throughput = 0.2
#clock induced charge is a noise factor that's introduced based on frequency of the readout. Faster clock = more noise. Have to set clock based upon brightnest thing in image (e.g. don't want Jupiters to saturate). Chris been assuming clock rate is set to something 10x as bright as exoEarth at quadrature is at saturation limit. Makes clock rate 10x higher than that.
#CIC = 1.3e-3 counts/pixel/photon count /clock cycle 
...
# Read-in new spectrum file only if changed
   ...
   #    two_nir = [940, 1040, 1180, 1310, 1480, 1690, 1800] 
    ...
    if comparison.value != lastcomparison or teststar:
      ...
      if comparison.value =='False O2 Planet':
        #  fn = 'fstarcloudy_geo_albedo.txt'
        #  fn = os.path.join(relpath, fn)
        #  model = np.loadtxt(fn, skiprows=0)
        #  lamhr_c = model[:,0]
        ...
        #  solhr_c =  cg.noise_routines.Fstar(lamhr_, Teff_, Rs_, semimajor.value, AU=True)
...
gain.callback = CustomJS(args=dict(source=source), code="""
    source.data = { value: [cb_obj.value] }
""")
throughput  = Slider(title="Aperture throughput", value = 0.2, start=0.05, end=1., step=0.05, callback_policy='mouseup') 
throughput.callback = CustomJS(args=dict(source=source), code="""
    source.data = { value: [cb_obj.value] }
""")
ssIWA  = Slider(title="Starshade IWA (mas)", value = 1, start=10, end=300, step=1, callback_policy='mouseup') 
ssIWA.callback = CustomJS(args=dict(source=source), code="""
    source.data = { value: [cb_obj.value] }
""")
ssOWA  = Slider(title="Starshade OWA (arcsec)", value = 1, start=100, end=1000, step=1, callback_policy='mouseup') 
ssOWA.callback = CustomJS(args=dict(source=source), code="""
    source.data = { value: [cb_obj.value] }
""")
#ground based choice
...
#bandpass choice
bandpass = Select(title="Show nominal LUVOIR bandpasses", value="No", options=["No",  "Yes"])
...
comparison = Select(title="Show comparison spectrum?", value ="none", options=["none", "Earth",  "Archean Earth", "Hazy Archean Earth", "1% PAL O2 Proterozoic Earth", "0.1% PAL O2 Proterozoic Earth","Venus", "Early Mars", "Mars", "Jupiter", "Saturn", "Uranus", "Neptune",'-----','Warm Neptune at 2 AU', 'Warm Neptune w/o Clouds at 1 AU', 'Warm Neptune w/ Clouds at 1 AU','Warm Jupiter at 0.8 AU', 'Warm Jupiter at 2 AU', "False O2 Planet", '-----', 'Proxima Cen b 10 bar 95% O2 dry', 'Proxima Cen b 10 bar 95% O2 wet', 'Proxima Cen b 10 bar O2-CO2', 'Proxima Cen b 90 bar O2-CO2', 'Proxima Cen b 90 bar Venus', 'Proxima Cen b 10 bar Venus', 'Proxima Cen b CO2/CO/O2 dry', 'Proxima Cen b Earth', 'Proxima Cen b Archean Earth', 'Proxima Cen b hazy Archean Earth', '-----','stars & galaxies:', '(arbitrary y units)', 'O5V star', 'B5V star', 'A5V star', 'F5V star', 'G2V star', 'G5V star', 'K2V star', 'M0V star', 'M2V star', 'M4V star', 'M5V star', 'Proxima Centauri star', 'T0 brown dwarf', 'T9 brown dwarf', 'L5 brown dwarf', 'L8 brown dwarf', 'NGC 337 spiral galaxy', 'NGC 660 peculiar galaxy', 'NGC 4621 elliptical galaxy', 'NGC 5033 spiral galaxy', 'Haro 6 blue compact dwarf galaxy', 'NGC 7476 spiral galaxy'])
...
ss = column(children=[starshade, ssIWA, ssOWA])
...
starshade_tab = Panel(child=ss, title='Starshade Option')
...
for sss in [starshade]:
    sss.on_change('value', update_data)

#inputs = Tabs(tabs=[ planet_tab, observation_tab, telescope_tab, instrument_tab, starshade_tab, time_tab, download_tab, info_tab ])
inputs = Tabs(tabs=[ planet_tab, observation_tab, telescope_tab, instrument_tab, time_tab, download_tab, info_tab ]) #starshade is not ready 
